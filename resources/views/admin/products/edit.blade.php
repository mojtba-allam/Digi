    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Update product details, pricing, and inventory settings.</p>
                </div>
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" class="p-6 space-y-6">
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input type="checkbox" 
                                       ...
                                       name="brands[]" 
                                       value="{{ $brand->id }}"
                                       {{ in_array($brand->id, old('brands', $product->brands->pluck('id')->toArray())) ? 'checked' : '' }}
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                            </div>
                            <div class="ml-2">
            ...
            <div class="border-t border-gray-200 pt-6">
                <h4 class="text-md font-medium text-gray-900 mb-4">Product Information</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Created</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $product->created_at->format('F j, Y \a\t g:i A') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Last Updated</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $product->updated_at->format('F j, Y \a\t g:i A') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Total Reviews</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $product->reviews()->count() }} reviews</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">In Carts</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $product->carts()->count() }} carts</p>
                    </div>
                </div>
            </div>
